...
generate_vars <- function(outcome, testdata){
  ...
  if (outcome == "ISUP") {
    ## make sure first column is always the label and class labels are always 1, 2, ...
    # Calibration Set
    df_calib <- select(df_calib, c(ISUP, X0:X2))
    df_calib$ISUP <- df_calib$ISUP+1
    # Test Set
    df_test <- select(df_test, c(ISUP, X0:X2))
...
df_predict_region <- function(pValues, sigfLevel, outcome){
  ...
  df <- within(df, {
    ...
    if (outcome == 'ISUP'){
      testLabels <- factor(testLabels,  levels = c(1,2,3),
                           labels = c('Rapid death', 'Persistent ill', 'Recovery'))
